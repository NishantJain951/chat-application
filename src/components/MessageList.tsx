...
export default function MessageList({ messages, isConversationLoading }: MessageListProps) {
      ...
      {isConversationLoading && messages.length > 0 && messages[messages.length-1].role === 'user' && (
        <div className="flex justify-start">
            <div className="max-w-[75%] sm:max-w-[70%] p-3 rounded-xl shadow-sm bg-muted italic flex items-center">
                <SymbolIcon className="h-4 w-4 animate-spin mr-2 inline-block" /> Assistant is thinking...
            </div>
        </div>
      )}
